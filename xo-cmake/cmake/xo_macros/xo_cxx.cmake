...
# ----------------------------------------------------------------
# xo_emit_python_wrapper(bindir)
#
# Write ${bindir}/xo-python: python3 with every pybind11 module built by THIS
# cmake configure already on PYTHONPATH.
#
# Collected from the global property xo_pybind11_module_dirs, appended by
# xo_pybind11_library().
#
# Call once, from the top-level CMakeLists of a build that configures the whole
# tree -- i.e. the umbrella.  A per-subsystem build would emit a script naming
# only the one module it builds, which is worse than no script: see below.
# Per-subsystem builds should use the installed ${PREFIX}/bin/xo-python, whose
...
function(xo_emit_python_wrapper bindir)
    ...
    if(NOT _dirs)
        message(STATUS "xo_emit_python_wrapper: no pybind11 modules;"
                       " skipping ${bindir}/xo-python")
        return()
    endif()
    ...
    # to be file(GENERATE) rather than configure_file/file(WRITE): they are only
    # resolvable at generate time.
    file(GENERATE
        OUTPUT ${bindir}/xo-python
        CONTENT
"#!/bin/sh
# GENERATED by xo_emit_python_wrapper() -- edits will be overwritten.
#
