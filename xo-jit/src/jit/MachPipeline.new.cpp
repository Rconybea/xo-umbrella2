...
namespace xo {
    ...
    namespace jit {
        ...
        namespace {
            ...
            llvm::PointerType *
            require_baseframe_ptr_llvm_type(xo::ref::brw<LlvmContext> llvm_cx)
            {
                llvm::StructType * baseframe_llvm_type
                    = require_baseframe_llvm_type(llvm_cx);

                return llvm::PointerType::getUnqual(baseframe_llvm_type);
            }
                                          ...
                                          llvm::IRBuilder<> & ir_builder)
        {
            ...
            /* generate function body -- code to execute later if/when function is called */
            ...
            llvm::AllocaInst * frame_alloca
                = create_entry_frame_alloca(llvm_fn,
                                            frame_llvm_type);
...

            {
                ...
                int i_slot = 0;
                int n_slot = 2 + lambda->n_arg();

                auto arg_ix = llvm_fn->arg_begin();

                for (i_slot = 0; i_slot < n_slot; ++i_slot) {
                    // TODO: move the frame-slot computation into helper function

                    /* argument i_slot-2 */
                    llvm::Value * frame_slot_ptr = nullptr;
                    {
                        /* note: we have to create instructions here because
                         *       the llvm IR is invariant w.r.t. data layout.
                         *       But we can't compute byte offsets until later
                         *       when data layout is revealed.
                         */
                    ...
                    if (i_slot == 0) {
                        ...
                        std::string arg_name = std::string(lambda->argv().at(i_arg)->name());

                        /* store param on function entry
                         *   see codegen_variable() for corresponding load
                         */
                        ir_builder.CreateStore(&(*arg_ix),
                                               frame_slot_ptr /*destination*/);
        ...
        /* frame pointer */
        llvm::Value *
        MachPipeline::codegen_global_frameptr(llvm::IRBuilder<> & ir_builder)
        {
            /* only want to jit this once */

            /* though really:  this would be once per thread */

            llvm::Type * baseframe_ptr_llvm_type
                = require_baseframe_ptr_llvm_type(llvm_cx_);

            /** nullptr - initial value for global frame pointer **/
            llvm::Constant * nullptr_llvm_value
                = llvm::ConstantPointerNull::get(baseframe_ptr_llvm_type);

            llvm::Value * fp_llvm_value
                = llvm::GlobalVariable(baseframe_ptr_llvm_type,
                                       false /*!isConstant*/
                                       llvm::GlobalValues::LinkOnceOdrLinkage,
                                       nullptr_llvm_value /*Initializer*/,
                                       "frame_pointer");

        } /*codegen_global_frameptr*/
                                       ...
                                       llvm::IRBuilder<> & ir_builder)
        {
            ...
            std::int32_t i_slot = env_stack_.top().lookup_var(var->name());
