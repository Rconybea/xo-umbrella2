 ...
 *
 *  ANSI foreground color for the ppsink pretty-printer: EMISSION.
 *
 *  The color values themselves (color_spec_type, color_encoding) and the
 *  process-wide gate (color_config) live in color_spec.hpp, which needs only a
 *  forward declaration of PpSink and so can be included by PpStyle.hpp.  This
 *  header includes it, so an existing `#include <xo/ppsink/color.hpp>` still
 *  gets everything it did.
 *
 *  Which color a tag or struct field is rendered in is NOT decided here: that
 *  is PpStyle, reached through PpSink::style() (PpStyle.hpp).
 *
