...
namespace xo {
    ...
    namespace print {
        ...
        void
        PrettyPrinter::connect_output(LogBuffer * p_out)
        {
            initial_indent_ = 0; //p_out->current_lpos();
            p_out_ = p_out;
        }

        void
        PrettyPrinter::put_cstr(const char * c_str)
        {
            // plan:
        ...
        void
        PrettyPrinter::check_print_ready()
        {
            // plan:
        ...
        uint32_t
        PrettyPrinter::count_visible_chars(const char * lo, const char * hi) const
        {
        ...
        void *
        PrettyPrinter::alloc(uint32_t z)
        {
        ...
        void *
        PrettyPrinter::expand_for(uint32_t z)
        {
            ...
            if (!tk_buffer_.expand(tk_buffer_.committed() + z, "PrettyPrinter::expand")) {
                assert(false);
                return nullptr;
            }
        ...
        void
        PrettyPrinter::reindex_stacks(const char * tk_dest,
                                      const char * tk_src,
                                      uint32_t z)
        {
