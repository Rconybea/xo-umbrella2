/** @file IPrintable_DUniqueString.cpp
 *
 *  Generated automagically from ingredients:
 *  1. code generator:
 *       [xo-facet/codegen/genfacet]
 *     arguments:
 *       --input [idl/IPrintable_DUniqueString.json5]
 *  2. jinja2 template for abstract facet .hpp file:
 *       [iface_facet_any.hpp.j2]
 *  3. idl for facet methods
 *       [idl/IPrintable_DUniqueString.json5]
**/

#include "uniquestring/IPrintable_DUniqueString.hpp"

namespace xo {
    namespace scm {
        auto
        IPrintable_DUniqueString::pretty_deprecated(const DUniqueString & self, const ppindentinfo & ppii) -> bool
        {
            return self.pretty_deprecated(ppii);
        }

        auto
        IPrintable_DUniqueString::pretty(const DUniqueString & self, PpSink & sink) -> void
        {
            self.pretty(sink);
        }


    } /*namespace scm*/
} /*namespace xo*/

/* end IPrintable_DUniqueString.cpp */
