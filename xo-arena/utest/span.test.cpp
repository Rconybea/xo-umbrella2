...
namespace xo {
    ...
    namespace ut {
        ...
        /* also for the byte-ish element types a hex dump would use */
        static_assert(std::ranges::contiguous_range<span<const char>>);
        static_assert(std::ranges::contiguous_range<span<std::uint8_t>>);

        TEST_CASE("span-range-walk", "[arena][span][ranges]")
        {
            char buf[] = "abcde";
        ...
        TEST_CASE("span-borrowed-range", "[arena][span][ranges]")
        {
            char buf[] = "xyz";
