/** @file IPrintable_DDeftypeSsm.cpp
 *
 *  Generated automagically from ingredients:
 *  1. code generator:
 *       [xo-facet/codegen/genfacet]
 *     arguments:
 *       --input [idl/IPrintable_DDeftypeSsm.json5]
 *  2. jinja2 template for abstract facet .hpp file:
 *       [iface_facet_any.hpp.j2]
 *  3. idl for facet methods
 *       [idl/IPrintable_DDeftypeSsm.json5]
**/

#include "deftype/IPrintable_DDeftypeSsm.hpp"

namespace xo {
    namespace scm {
        auto
        IPrintable_DDeftypeSsm::pretty_deprecated(const DDeftypeSsm & self, const ppindentinfo & ppii) -> bool
        {
            return self.pretty_deprecated(ppii);
        }

        auto
        IPrintable_DDeftypeSsm::pretty(const DDeftypeSsm & self, PpSink & sink) -> void
        {
            self.pretty(sink);
        }


    } /*namespace scm*/
} /*namespace xo*/

/* end IPrintable_DDeftypeSsm.cpp */
