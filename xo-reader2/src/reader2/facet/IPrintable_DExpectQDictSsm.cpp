/** @file IPrintable_DExpectQDictSsm.cpp
 *
 *  Generated automagically from ingredients:
 *  1. code generator:
 *       [xo-facet/codegen/genfacet]
 *     arguments:
 *       --input [idl/IPrintable_DExpectQDictSsm.json5]
 *  2. jinja2 template for abstract facet .hpp file:
 *       [iface_facet_any.hpp.j2]
 *  3. idl for facet methods
 *       [idl/IPrintable_DExpectQDictSsm.json5]
**/

#include "expect_qdict/IPrintable_DExpectQDictSsm.hpp"

namespace xo {
    namespace scm {
        auto
        IPrintable_DExpectQDictSsm::pretty_deprecated(const DExpectQDictSsm & self, const ppindentinfo & ppii) -> bool
        {
            return self.pretty_deprecated(ppii);
        }

        auto
        IPrintable_DExpectQDictSsm::pretty(const DExpectQDictSsm & self, PpSink & sink) -> void
        {
            self.pretty(sink);
        }


    } /*namespace scm*/
} /*namespace xo*/

/* end IPrintable_DExpectQDictSsm.cpp */
