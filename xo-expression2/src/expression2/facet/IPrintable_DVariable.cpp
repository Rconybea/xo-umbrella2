...
namespace xo {
    namespace scm {
        auto
        IPrintable_DVariable::pretty_deprecated(const DVariable & self, const ppindentinfo & ppii) -> bool
        {
            return self.pretty_deprecated(ppii);
        }

        auto
        IPrintable_DVariable::pretty(const DVariable & self, PpSink & sink) -> void
        {
            self.pretty(sink);
        }
