...
namespace xo {
    namespace scm {
        auto
        IPrintable_DConstant::pretty_deprecated(const DConstant & self, const ppindentinfo & ppii) -> bool
        {
            return self.pretty_deprecated(ppii);
        }

        auto
        IPrintable_DConstant::pretty(const DConstant & self, PpSink & sink) -> void
        {
            self.pretty(sink);
        }
