/** @file IPrintable_DPrimitive_gco_1_gco.cpp
 *
 *  Generated automagically from ingredients:
 *  1. code generator:
 *       [xo-facet/codegen/genfacet]
 *     arguments:
 *       --input [idl/IPrintable_DPrimitive_gco_1_gco.json5]
 *  2. jinja2 template for abstract facet .hpp file:
 *       [iface_facet_any.hpp.j2]
 *  3. idl for facet methods
 *       [idl/IPrintable_DPrimitive_gco_1_gco.json5]
**/

#include "detail/IPrintable_DPrimitive_gco_1_gco.hpp"

namespace xo {
    namespace scm {
        auto
        IPrintable_DPrimitive_gco_1_gco::pretty_deprecated(const DPrimitive_gco_1_gco & self, const ppindentinfo & ppii) -> bool
        {
            return self.pretty_deprecated(ppii);
        }

        auto
        IPrintable_DPrimitive_gco_1_gco::pretty(const DPrimitive_gco_1_gco & self, PpSink & sink) -> void
        {
            self.pretty(sink);
        }


    } /*namespace scm*/
} /*namespace xo*/

/* end IPrintable_DPrimitive_gco_1_gco.cpp */
